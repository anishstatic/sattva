...
import { BookOpenCheck, Eraser, FileText, GitPullRequest, Hash, House, Image, ImageUpscale, LogOut, Scissors, SquarePen,  Users } from 'lucide-react'
...
const navItems = [
    ...
    {to: '/ai/ai-dictionary', label: 'AI Dictionary', Icon: BookOpenCheck},
            ...
            <div className='px-6 mt-5 text-sm text-gray-600 font-medium'>
                {navItems.map(({to, label, Icon})=>(
                    ...
                    className={({isActive})=> `px-3.5 py-2.5 flex items-center gap-3 rounded ${isActive ? 'bg-gradient-to-r from-[#3C81F6] to-[#9234EA] text-white' : ''}`} >
                        {({ isActive })=>(
                            <>
                            <Icon className={`w-4 h-4 ${isActive ? 'test-white' : ''}`} />
                            {label}
                            </>
                        )}
                    </NavLink>
