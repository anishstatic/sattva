...
const BlogTitles = () => {
  ...
  const blogCategories = [
     'General', 'Technology', 'Business', 'Health', 'Lifestyle', 'Eductaion', 'Travel', 'Food'
    ]
    ...
    const onSubmitHandler = async (e)=>{
      ...
      try {
        setLoading(true)
        const prompt = `Generate blog title for the keyword ${input} in the category of ${selectedCategory}`
  ...
  return (
    <div className='h-full overflow-y-scroll p-6 flex items-start flex-wrap gap-4 text-slate-700'>
      ...
      <form onSubmit={onSubmitHandler} className='w-full max-w-lg p-4 bg-white rounded-lg border border-gray-200'>
        ...
        <input onChange={(e)=> setInput(e.target.value)} value={input} type="text" className='w-full p-2 px-3 outline-none text-sm rounded-md border border-gray-300' placeholder='The Future of Artificial intelligence is ....' required />
